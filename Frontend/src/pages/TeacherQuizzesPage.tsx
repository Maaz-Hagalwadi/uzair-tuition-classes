  ...
  return (
    <DashboardShell navItems={TEACHER_NAV}>
      <div className="max-w-4xl mx-auto">
        ...
        {/* Batch filter */}
        {batches.length > 1 && (
          <div className="flex gap-1 mb-5 bg-[#f3f4f6] p-1 rounded-lg w-fit overflow-x-auto">
            <button onClick={() => setBatchFilter('ALL')}
              className={`px-3 py-1.5 rounded-md text-xs font-medium whitespace-nowrap transition-colors ${
                batchFilter === 'ALL' ? 'bg-white text-[#1e1b4b] shadow-sm' : 'text-[#6b7280] hover:text-[#374151]'
              }`}>
            ...
            {batches.map(b => (
              <button key={b.id} onClick={() => setBatchFilter(b.id)}
                className={`px-3 py-1.5 rounded-md text-xs font-medium whitespace-nowrap transition-colors ${
                  batchFilter === b.id ? 'bg-white text-[#1e1b4b] shadow-sm' : 'text-[#6b7280] hover:text-[#374151]'
                }`}>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-[#9ca3af]">
            ...
            <p className="text-sm font-medium">No batches assigned yet</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-[#9ca3af]">
            <span className="material-symbols-outlined text-[48px] mb-3" style={{ fontVariationSettings: "'FILL' 1" }}>quiz</span>
            <p className="text-sm font-medium">No quizzes yet</p>
            <p className="text-xs mt-1">Click "New Quiz" to create your first quiz</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {filtered.map(quiz => {
              ...
              return (
                <div key={quiz.id} className="bg-white rounded-xl border border-[#e4e2e6] p-5 flex flex-col gap-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-[#1e1b4b] text-sm">{quiz.title}</p>
                        <p className="text-xs text-[#9ca3af]">{quiz.batchName}</p>
                      </div>
                    </div>
                  ...
                  {quiz.description && (
                    <p className="text-xs text-[#6b7280] line-clamp-2">{quiz.description}</p>
                  )}
                  ...
                  <div className="flex items-center gap-2 pt-1 border-t border-[#f3f4f6]">
                    <Link to={`/teacher/quizzes/${quiz.id}`}
                      className="flex-1 flex items-center justify-center gap-1 px-3 py-1.5 bg-[#eef2ff] text-[#6366f1] rounded-lg text-xs font-medium hover:bg-[#e0e7ff] transition-colors">
